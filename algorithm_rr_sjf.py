"""
Implementación del algoritmo Round Robin con SJF (Shortest Job First).
Los nuevos procesos que llegan se insertan en la cola de listos según SJF,
...
def schedule_round_robin_sjf(processes, quantum=2):
    """
    Implementa el algoritmo Round Robin con inserción SJF.
    Los nuevos procesos se insertan según SJF, pero el orden de ejecución
    ...
    # Cola de procesos listos (SJF - se ordenan por tiempo de ráfaga)
    ready_queue = []
    current_time = 0
    completed = 0
    execution_order = []
    ...
    while completed < len(processes):
        ...
        # Los nuevos procesos se insertan según SJF
        new_arrivals = []
        ...
        # Insertar nuevos procesos en orden SJF
        if new_arrivals:
            new_arrivals.sort(key=lambda p: p.burst_time)
    ...
    results.algorithm_title = f"Round Robin + SJF (Q={quantum})"
